...
class OrthogonalDecisionTree:
    """
    Arvore usada dentro da Random Forest ortogonal.

    Cada no ainda e ortogonal: escolhe uma feature i* e um limiar tau*.
    A diferenca para a arvore simples e que, em cada no, a busca usa apenas
    um subconjunto aleatorio de features, como em Random Forest tradicional.
    """
...
class OrthogonalRandomForest:
    """
    Random Forest ortogonal implementada do zero com NumPy.

    Para cada arvore:
    1. Sorteia uma amostra bootstrap do treino.
    2. Treina uma arvore ortogonal nessa amostra.
    3. Na predicao, combina as arvores por voto majoritario.
    """
    ...
    def __init__(
        ...
        self.rng = np.random.default_rng(random_state)
        self.trees: list[OrthogonalDecisionTree] = []
        self.n_classes_: int = 0

    def fit(self, X: np.ndarray, y: np.ndarray) -> "OrthogonalRandomForest":
        X = np.asarray(X, dtype=float)
        y = np.asarray(y, dtype=int)
        self.n_classes_ = int(np.max(y)) + 1
        self.trees = []
...
def main() -> None:
    ...
    final_model = OrthogonalRandomForest(
        n_estimators=50,
        max_depth=12,
        min_samples_split=10,
        min_samples_leaf=4,
    ...
    save_predictions("predicoes_random_forest_ortogonal.csv", test_predictions)
    print("Predicoes salvas em predicoes_random_forest_ortogonal.csv")
